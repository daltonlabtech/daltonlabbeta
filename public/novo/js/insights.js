...
(function () {
  ...
  const POSTS = [
    /* ---------------- TEAM INSIGHT — Rodrigo / Karpathy ---------------- */
    {
      ...
      art: "art-1",
      title: t(
        "O que Karpathy ensinou — sem querer — para quem lidera uma empresa",
        "What Karpathy taught — unintentionally — for anyone leading a company"
      ),
      dek: t(
  ...
  function liItem(post) {
    ...
    a.dataset.search = (L(post.title) + " " + L(post.cat) + " " + L(post.dek)).toLowerCase();
    ...
    a.innerHTML = `<div class="ip-row-main">${meta}${title}${byline}</div><span class="ip-go" aria-hidden="true">→</span>`;
    a.addEventListener("click", (e) => { e.preventDefault(); openReader(post.id); });
    return a;
  }
  ...
  function relatedHTML(current) {
    ...
    const items = others.map((p) => `
      <a class="ir-rel" data-id="${p.id}" href="#${p.id}">
        <div class="ip-meta"><span>${L(p.date)}</span></div>
        <div class="ir-rel-title">${L(p.title)}</div>
  ...
  function openReader(id) {
    ...
    const pdf = post.pdf ? `<button class="btn btn-primary ir-pdf" id="ir-pdf">
        <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12m0 0l-4-4m4 4l4-4M5 21h14"/></svg>
        ${D("insp.pdf")}</button>` : "";
