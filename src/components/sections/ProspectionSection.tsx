...
const agentData: Record<AgentTab, AgentContent> = {
  vendas: {
    ...
    features: ['Respostas instantâneas', 'Entende o seu cliente ideal e o direciona para reuniões', 'Guia o cliente no processo da compra de forma autônoma'],
  ...
  conteudo: {
    ...
    features: ['Analisa sua marca e entende sua identidade', 'Automatiza a criação de conteúdo para as suas redes sociais', 'Personaliza os conteúdos com base no perfil da sua marca e público'],
  ...
  anuncio: {
    ...
    features: ['Realiza testes em massa para encontrar seu público ideal', 'Analisa dados em tempo real para identificar tendências de performance', 'Aplica melhorias de performance para maximizar ROI'],
    ctaText: 'Lista de espera',
    ctaAction: 'modal',
    isComingSoon: true
  }
        ...
        <div className="bg-[#F5F3F0] rounded-3xl p-6 md:p-10 lg:p-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center min-h-[400px]">
            ...
            <div className={`${revealClasses(isVisible)}`} style={getStaggerDelay(1)}>
              {/* Title */}
              <h3 className="font-inter font-bold text-2xl md:text-3xl text-zinc-900 leading-tight mb-4 lg:text-3xl">
                {currentAgent.title}
              </h3>
              ...
              <ul className="space-y-3 mb-6">
                {currentAgent.features.map((feature, index) => <li key={index} className="flex items-start gap-3">
              ...
              {/* CTA Button */}
              <button onClick={handleCtaClick} className="inline-flex items-center justify-center font-medium text-sm px-6 py-3 rounded-full transition-all duration-300 relative overflow-hidden hover:opacity-90" style={{
            ...
            <div className={`relative hidden lg:flex items-center justify-center ${revealClasses(isVisible)}`} style={getStaggerDelay(2)}>
              <div className="relative w-full max-w-[320px] mx-auto aspect-square">
                {currentAgent.isComingSoon ? (/* Coming Soon Background with Blur Image - All corners rounded - Very Light Monochromatic */
              <div className="w-full h-full rounded-2xl overflow-hidden relative">
                    <img src={blurBackground} alt="" className="absolute inset-0 w-full h-full object-cover rounded-2xl grayscale brightness-150 contrast-50 opacity-70" />
                    {/* Coming Soon Overlay */}
                  ...
                  </div>) : (/* Dashboard Mockup - Sales Preview */
...
const SalesDashboardMockup = () => <div className="bg-[#e8e6e3] rounded-2xl overflow-hidden h-full flex flex-col p-4 justify-center relative">
    ...
    <svg className="w-full h-full" viewBox="0 0 280 180" preserveAspectRatio="xMidYMid meet">
      ...
      {/* Vertical grid lines */}
      {[0, 1, 2, 3, 4, 5, 6].map(i => <line key={i} x1={45 + i * 35} y1="25" x2={45 + i * 35} y2="145" stroke="#d4d4d8" strokeOpacity="0.3" strokeWidth="1" />)}
      
      {/* X-axis labels (S, T, Q, Q, S, S, D) */}
      {['S', 'T', 'Q', 'Q', 'S', 'S', 'D'].map((day, i) => <text key={i} x={45 + i * 35} y="160" fontSize="10" fill="#71717a" textAnchor="middle" fontFamily="Inter, sans-serif">
          {day}
        </text>)}
